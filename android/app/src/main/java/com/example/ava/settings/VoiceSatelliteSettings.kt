...
private const val APPLIANCE_SERVER_PORT = 6503
...
private val DEFAULT = VoiceSatelliteSettings(
    ...
    serverPort = APPLIANCE_SERVER_PORT,
    ...
    suspend fun ensureMacAddressIsSet() {
        update {
            if (it.macAddress == DEFAULT_MAC_ADDRESS) {
                ...
                it.copy(macAddress = newMac, name = uniqueName)
            } else if (it.serverPort == 6053) {
                it.copy(serverPort = generateUniquePort())
            } else {
                it
            }
        }
