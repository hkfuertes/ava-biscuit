...
class ExternalSoundResolverTest {
    ...
    fun usesMatchingExternalSoundForBundledSound() {
        ...
        File(dir, "bubble.mp3").writeBytes(byteArrayOf(1))
        ...
        assertEquals(
            "file://${File(dir, "bubble.mp3").absolutePath}",
            ExternalSoundResolver.resolve("asset:///sounds/bubble.mp3", dir)
        )
    }
    ...
    fun keepsBundledSoundWhenExternalFileIsMissingOrUnsafe() {
        ...
        assertEquals("file:///tmp/old.wav", ExternalSoundResolver.resolve("file:///tmp/old.wav", dir))
        assertTrue(ExternalSoundResolver.isSafeSoundName("stop_sound.wav"))
        assertTrue(ExternalSoundResolver.isSafeSoundName("bubble.mp3"))
